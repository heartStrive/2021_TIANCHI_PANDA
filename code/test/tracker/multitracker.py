...
class JDE_PANDA_Tracker(object):
    def __init__(self, opt, frame_rate=30):
        self.opt = opt
    ...
    def update(self, dets, feat):
        ...
        t1 = time.time()
        if len(dets)>0:
            detections = [STrack(tlbrs[:4], tlbrs[4], f, 30) for
                          (tlbrs, f) in zip(dets[:, :5], feat)]
        else:
            detections = []


        t2 = time.time()
        print('3    Forward cost time: {} s'.format(t2 - t1))
        ...
        for track in self.lost_stracks:
            if self.frame_id - track.end_frame > self.max_time_lost:
                ...
                removed_stracks.append(track)
        t4 = time.time()
        print('4    Ramained match cost time: {} s'.format(t4 - t3))
