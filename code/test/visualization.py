...
def plot_tracking_panda(image, tlwhs, obj_ids, scores=None, frame_id=0, fps=0.0, ids2=None, scales=(1,)):
    src_h, src_w = image.shape[:2]
    image = cv2.resize(image, (int(src_w * scales), int(src_h * scales)))
    im = np.ascontiguousarray(np.copy(image))
    im_h, im_w = im.shape[:2]
    ...
    for i, tlwh in enumerate(tlwhs):
        ...
        line_thickness = 3
        cv2.rectangle(
            im, (int(intbox[0] * scales), int(intbox[1] * scales)), (int(intbox[2] * scales), int(intbox[3] * scales)), color=(0,0,255), thickness=line_thickness
        )

        cv2.putText(
            im,
            id_text,
            (int(intbox[0] * scales), int((intbox[1] + 30) * scales)),
            cv2.FONT_HERSHEY_PLAIN,
            text_scale,
            (0, 0, 255),
            thickness=text_thickness,
